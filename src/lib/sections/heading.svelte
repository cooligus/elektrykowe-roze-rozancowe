<script>
    import mary from "$lib/assets/mary.svg"
</script>

    <div class="w-screen h-screen flex justify-center items-center bg-gradient-to-r from-yellow-500 via-cyan-600 to-yellow-500 z-0">
      <div class="w-screen h-100 absolute z-0 flex justify-center overflow-hidden">
        <img
        class="md:w-screen min-w-[40em]"
        src={mary}
        alt="Picture of saint dominic getting rossary"
        />
      </div>
      <div class="z-10 bg-zinc-900/80 rounded-xl py-5 px-7 text-center drop-shadow-2xl backdrop-blur-lg">
        <h1 class="md:text-6xl text-3xl text-rose-600 uppercase font-extrabold">
          Eletrykowe <br/>
          Róże Różańcowe
        </h1>
        <p class="text-white md:text-2xl text-lg font-light">
            Modlisz się jednym dziesiątkiem, <br/>
            a róża odmawia resztę
        </p>
      </div>
    </div>